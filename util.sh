...
function create-nodes() {
  ...
  for ((i=1; i<=$NUMZONES; i++)); do	
  #TODO: parallelize this loop to speed up the process
    local group_name="${NODE_INSTANCE_PREFIX}-group"
#    if [[ $i == ${NUMZONES} ]]; then
...
#    fi
    REQZONE=$(echo "${ALLZONES}" | sed -n ${i}p)
    # Spread the remaining number of nodes evenly
    this_mig_size=${NUMZONES}
    this_mig_size=$((${instances_left} / (${NUMZONES}-${i}+1)))
...
#LOOP TILL NUMZONES REACHED; This was created with the regional scaling capability
  for ((i=1; i<=${NUMZONES}; i++)); do
    local group_name="${NODE_INSTANCE_PREFIX}-group"
    if [[ $i == ${NUM_MIGS} ]]; then
      # TODO: We don't add a suffix for the last group to keep backward compatibility when there's only one MIG.
    ...
    REQZONE=$(echo "${ALLZONES}" | sed -n ${i}p)
    this_mig_min=$((${left_min}/(${NUMZONES}-${i}+1)))
    this_mig_max=$((${left_max}/(${NUMZONES}-${i}+1)))
    left_min=$((left_min-$this_mig_min))
    left_max=$((left_max-$this_mig_max))
    echo "INSTANCE GROUP ZONES : $REQZONE"
    echo "THIS MIG MIN : $this_mig_min & THIS MIG MAX : $this_mig_max"
    local mig_url="https://www.googleapis.com/compute/v1/projects/${PROJECT}/zones/${REQZONE}/instanceGroups/${group_name}"
