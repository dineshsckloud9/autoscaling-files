...
NODE_DISK_SIZE=${NODE_DISK_SIZE:-10GB}
...
NUMZONES=$($GCLOUD compute zones list --filter="region:${REGION}" | awk '{ print $1 }' | sed '1d' | wc -l)
ALLZONES=$($GCLOUD compute zones list --filter="region:${REGION}" | awk '{ print $1 }' | sed '1d')	   
...
if [[ "${ENABLE_CLUSTER_AUTOSCALER}" == "true" ]]; then
  AUTOSCALER_MIN_NODES="${KUBE_AUTOSCALER_MIN_NODES:-3}"
  AUTOSCALER_MAX_NODES="${KUBE_AUTOSCALER_MAX_NODES:-7}"
  AUTOSCALER_ENABLE_SCALE_DOWN="${KUBE_AUTOSCALER_ENABLE_SCALE_DOWN:-true}"
  AUTOSCALER_EXPANDER_CONFIG="${KUBE_AUTOSCALER_EXPANDER_CONFIG:---expander=random}"
fi
